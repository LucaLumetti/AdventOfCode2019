const fs = require('fs')
let input = fs.readFileSync('input2', 'utf-8').trim().split('\n').map(l => l.split(''))
...
function qdr(x, y){
    let mida = Math.ceil(input.length/2-0.5)
    let midb = Math.ceil(input[0].length/2-0.5)
    let r
    if(x <= mida && y <= midb)
        r=1
    if(x <= mida && y > midb)
...
function takeKey(map, k){
    ...
    map.forEach((r, i) => {
        r.forEach((c, j) => {
            if(c === '@' && qdr(kx, ky) === qdr(i, j)) map[i][j] = '.'
        })
    })
    map[kx][ky] = '@'
    let d = k.toUpperCase()
